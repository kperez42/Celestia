...
class BackgroundCheckManager {
    ...
    @Published var checkInProgress: Bool = false
    ...
    // MARK: - Background Check Methods

    /// Initiate background check for user
    func performBackgroundCheck(userId: String, consent: Bool) async throws -> BackgroundCheckResult {
        guard consent else {
        ...
        Logger.shared.info("Starting background check for user: \(userId)", category: .general)
        ...
        Logger.shared.info("Background check completed for user: \(userId)", category: .general)
